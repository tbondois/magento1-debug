...
/**
 * Class TBD_Debug_Model_Observer
 *
 * @category TBD
 * @package  TBD_Debug
...
class TBD_Debug_Model_Observer
{
    ...
    public function getRequestInfo()
    {
        if ($this->requestInfo === null) {
            $this->requestInfo = Mage::getModel('tbd_debug/requestInfo');
        }
    ...
    public function startProfiling()
    {
        ...
        $this->canCapture = Mage::helper('tbd_debug')->canCapture();
        ...
        if (Mage::helper('tbd_debug')->canEnableVarienProfiler()) {
    ...
    public function updateProfiling()
    {
        ...
        // save rendering time
        $requestInfo->setRenderingTime(TBD_Debug_Model_Block::getTotalRenderingTime());
    ...
    {
        if (!$this->canCollect() || !Mage::helper('tbd_debug')->canPersist()) {
            return;
        }
    ...
    public function onLayoutGenerate(Varien_Event_Observer $observer)
    {
        ...
        foreach ($layoutBlocks as $block) {
            if (!$this->canCaptureBlock($block)
                //@fixes exclusions for the project :
                || get_class($block) === 'Mage_Wishlist_Block_Customer_Wishlist_Item_Options'
            ) {
                continue;
            }
            // @fixes The module make a page crashing ? uncomment the var_dump() below and add the last displayed class at L225,
    ...
    public function onBlockToHtml(Varien_Event_Observer $observer)
    {
        ...
        $blockName = Mage::helper('tbd_debug')->getBlockName($block);
    ...
    public function onWebsiteRestriction(Varien_Event_Observer $observer)
    {
        ...
        $helper = Mage::helper('tbd_debug');
        if ($helper->canShowToolbar() && $controller instanceof TBD_Debug_Controller_Front_Action) {
    ...
    public function canCaptureBlock($block)
    {
        ...
        if (strpos($blockClass, 'TBD_Debug_Block') > 0) {
            return false;
        }
