...
class TBD_Debug_Model_Service
{
    ...
    {
        $this->getConfig()->saveConfig(TBD_Debug_Helper_Data::DEBUG_OPTION_FORCE_VARIEN_PROFILE_PATH, (int)$isEnabled);
    }
    ...
    {
        if (!Mage::helper('tbd_debug')->isMagentoEE()) {
    ...
    public function searchConfig($query)
    {
        ...
        $configArray = Mage::helper('tbd_debug')->xml2array($this->getConfig()->getNode(), $configArray);
    ...
    {
        $table = Mage::getResourceModel('tbd_debug/requestInfo')->getMainTable();
    ...
    public function getFileUpdatesWithHandle($handle, $storeId, $area)
    {
        ...
        $updateFiles = Mage::helper('tbd_debug')->getLayoutUpdatesFiles($storeId, $area);
