...
/**
 * Class TBD_Debug_Test_Model_Service
 *
 * @category TBD
 ...
 *
 * @covers TBD_Debug_Model_Service
 * @codeCoverageIgnore
 */
class TBD_Debug_Test_Model_Service extends EcomDev_PHPUnit_Test_Case
{

    public function testConstruct()
    {
        $model = Mage::getModel('tbd_debug/service');
        $this->assertNotFalse($model);
        $this->assertInstanceOf('TBD_Debug_Model_Service', $model);
    }
    ...
    public function testFlushCache()
    {
        ...
        $model = $this->getModelMock('tbd_debug/service', array('getCacheInstance'));
    ...
    public function testGetModuleConfigFilePath()
    {
        ...
        $configMock->expects($this->once())->method('getModuleConfig')
            ->with('TBD_Module')->willReturn(new Varien_Simplexml_Element('<config><a/></config>'));

        $model = $this->getModelMock('tbd_debug/service', array('getConfig'));
        $model->expects($this->any())->method('getConfig')->willReturn($configMock);

        $actual = $model->getModuleConfigFilePath('TBD_Module');
        $this->assertEquals('etc/modules/TBD_Module.xml', $actual);
    }
    ...
    public function testGetModuleConfigFilePathNotFound()
    {
        ...
        $configMock->expects($this->any())->method('getOptions')->willReturn($configOptions);
        $configMock->expects($this->once())->method('getModuleConfig')
            ->with('TBD_Module')->willReturn(null);

        $model = $this->getModelMock('tbd_debug/service', array('getConfig'));
        $model->expects($this->any())->method('getConfig')->willReturn($configMock);

        $model->getModuleConfigFilePath('TBD_Module');
    }


    public function testSetModuleStatus()
    {
        $configXml = (object)(array('modules' => (object)array('TBD_Module' => (object)array('version' => true))));

        $model = $this->getModelMock('tbd_debug/service', array('getModuleConfigFilePath', 'loadXmlFile', 'saveXml'));
        $model->expects($this->once())->method('getModuleConfigFilePath')->with('TBD_Module')->willReturn('etc/TBD_Module.xml');
        $model->expects($this->once())->method('loadXmlFile')->with('etc/TBD_Module.xml')->willReturn($configXml);
        $model->expects($this->once())->method('saveXml')->with($configXml, 'etc/TBD_Module.xml')->willReturn(true);

        $model->setModuleStatus('TBD_Module', false);
        $this->assertEquals('false', (string)$configXml->modules->TBD_Module->active);
    }
    ...
    public function testSetModuleStatusWithoutPermissions()
    {
        ...
        $model->expects($this->once())->method('getModuleConfigFilePath')->with('TBD_Module')->willReturn('etc/TBD_Module.xml');
        $model->expects($this->once())->method('loadXmlFile')->with('etc/TBD_Module.xml')->willReturn($configXml);
        $model->expects($this->once())->method('saveXml')->with($configXml, 'etc/TBD_Module.xml')->willReturn(false);
    ...
    {
        $actual = Mage::getModel('tbd_debug/service')->getLocalXmlFilePath();
    ...
    public function testSetSqlProfilerStatusDisable()
    {
        ...
        $model = $this->getModelMock('tbd_debug/service', array('getLocalXmlFilePath', 'loadXmlFile', 'saveXml'));
    ...
    public function testVarienProfilerStatus()
    {
        ...
        $configMock->expects($this->once())->method('saveConfig')
            ->with('tbd_debug/options/force_varien_profile', 1);
    ...
    {
        $helper = $this->getHelperMock('tbd_debug', array('isMagentoEE'));
    ...
    public function testSetTemplateHints()
    {
        ...
        $model = $this->getModelMock('tbd_debug/service', array('getConfig', 'deleteTemplateHintsDbConfigs'));
        $model->expects($this->any())->method('getConfig')->willReturn($configMock);
    ...
    public function testSearchConfig()
    {
        ...
        $helper = $this->getHelperMock('tbd_debug', array('xml2array'));
        $this->replaceByMock('helper', 'tbd_debug', $helper);
        $helper->expects($this->once())->method('xml2array')
            ->with($configNodeElement)
            ->willReturn(array(
                'modules/TBD_Module/version'   => '1.0.0',
                'events/cool'                    => '1',
                'tbd_module/events/some_event' => '2',
                'crontab/tbd/jobs/tbd'       => 'ok'
            ));
        ...
        $this->assertArrayHasKey('tbd_module/events/some_event', $actual);
    }
    ...
    public function testDeleteTemplateHintsDbConfigs()
    {
        ...
        Mage::getModel('tbd_debug/service')->deleteTemplateHintsDbConfigs();
    }
    ...
    public function testPurgeAllProfiles()
    {
        ...
        $actual = Mage::getModel('tbd_debug/service')->purgeAllProfiles();
        $this->assertEquals(2, $actual);
    }


    /**
     * @covers TBD_Debug_Model_Service::getFileUpdatesWithHandle
     */
...
XML;
        ...
        $helperMock = $this->getHelperMock('tbd_debug', array('getLayoutUpdatesFiles'));
        ...
        $model = $this->getModelMock('tbd_debug/service', array('loadXmlFile'));
    ...
    /**
     * @covers TBD_Debug_Model_Service::getDatabaseUpdatesWithHandle
     */
    public function testGetDatabaseUpdatesWithHandle()
    {
        ...
        $actual = Mage::getModel('tbd_debug/service')->getDatabaseUpdatesWithHandle('some_handle', 10, 'adminhtml');
        $this->assertCount(2, $actual);
