...
/**
 * Class TBD_Debug_Test_Helper_Data
 *
 * @category TBD
 * @package  TBD_Debug
 ...
 *
 * @covers TBD_Debug_Helper_Data
 */
class TBD_Debug_Test_Helper_Data extends EcomDev_PHPUnit_Test_Case
{
    ...
    {
        $this->assertEquals('TBD_Debug', $this->helper->getModuleName());
    }
    ...
    public function testGetModuleDirectory()
    {
        ...
        $config->expects($this->once())->method('getModuleDir')->with('', 'TBD_Module')->willReturn('app/code/community');

        $helper = $this->getHelperMock('tbd_debug', array('getModuleName', 'getConfig'));
        $helper->expects($this->any())->method('getModuleName')->willReturn('TBD_Module');
...
XML;
        ...
        $this->assertArrayHasKey('root/modules/TBD_Debug/version', $data);
        $this->assertEquals('1.3.1', $data['root/modules/TBD_Debug/version']);
        $this->assertArrayHasKey('root/global/models/tbd_debug/class', $data);
        $this->assertEquals('TBD_Debug_Model', $data['root/global/models/tbd_debug/class']);
    }
    ...
    public function testCanCapture($isDebugRequest, $canShowToolbar, $expected)
    {
        ...
        $helper = $this->getHelperMock('tbd_debug', array('_getRequest', 'isTBDDebugRequest', 'canShowToolbar'));
        $helper->expects($this->any())->method('_getRequest')->willReturn($request);
        $helper->expects($this->once())->method('isTBDDebugRequest')->with($request)->willReturn($isDebugRequest);
    ...
    public function testIsTBDDebugRequest()
    {
        $request = $this->getMock('Mage_Core_Controller_Request_Http', array(), array(), '', false);
        $request->expects($this->once())->method('getPathInfo')->willReturn('/tbd_debug/');

        $this->assertTrue($this->helper->isTBDDebugRequest($request));
    }

    public function testIsTBDDebugRequestFalse()
    {
        $request = $this->getMock('Mage_Core_Controller_Request_Http', array(), array(), '', false);
        $request->expects($this->once())->method('getPathInfo')->willReturn('/catalog/product/view/');

        $this->assertFalse($this->helper->isTBDDebugRequest($request));
    }

    public function testCanShowToolbarWithoutDevMode()
    {
        Mage::setIsDeveloperMode(false);
        $helper = $this->getHelperMock('tbd_debug', array('isEnabled', 'getIsDeveloperMode', 'isDevAllowed'));
    ...
    {
        $helper = $this->getHelperMock('tbd_debug', array('canShowToolbar'));
...
XML;
        ...
        $helper = $this->getHelperMock('tbd_debug', array('getConfig'));
        $helper->expects($this->any())->method('getConfig')->willReturn($config);
