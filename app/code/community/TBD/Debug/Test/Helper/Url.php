...
/**
 * Class TBD_Debug_Test_Helper_Url
 *
 * @category TBD
 * @package  TBD_Debug
 ...
 *
 * @covers TBD_Debug_Helper_Url
 * @codeCoverageIgnore
 */
class TBD_Debug_Test_Helper_Url extends EcomDev_PHPUnit_Test_Case
{
    /** @var TBD_Debug_Helper_Url */
    protected $helper;

    protected function setUp()
    {
        $this->helper = $this->getHelperMock('tbd_debug/url', array('getUrl'));
    }
    ...
    {
        $helper = $this->getHelperMock('tbd_debug/url', array('getRouteStoreId', '_getUrl'));
    ...
    {
        $this->helper->expects($this->once())->method('getUrl')->with('module/enable', array('module' => 'TBD_Debug'));
        $this->helper->getEnableModuleUrl('TBD_Debug');
    }


    public function testGetDisableModuleUrl()
    {
        $this->helper->expects($this->once())->method('getUrl')->with('module/disable', array('module' => 'TBD_Debug'));
        $this->helper->getDisableModuleUrl('TBD_Debug');
    }
    ...
    {
        $this->helper->expects($this->once())->method('getUrl')->with('block/viewBlock', array('block' => 'TBD_Debug_Block_Panel'));
        $this->helper->getViewBlockUrl('TBD_Debug_Block_Panel');
    }


    public function testGetViewTemplateUrl()
    {
        $encodedTemplate = strtr(base64_encode('tbd_debug/toolbar.phtml'), '+/=', '-_,');

        $this->helper->expects($this->once())->method('getUrl')->with('block/viewTemplate', array('template' => $encodedTemplate));
        $this->helper->getViewTemplateUrl('tbd_debug/toolbar.phtml');
    }
